...
export default function DJIA() {
  ...
  const sendMessage = async (msg) => {
    ...
    const deckAInfo = deckA.track ? (' + deckA.track.title +  (' + (deckA.isPlaying?'SONANDO':'pausa') + ')') : 'vacío';
    const deckBInfo = deckB.track ? (' + deckB.track.title +  (' + (deckB.isPlaying?'SONANDO':'pausa') + ')') : 'vacío';
    const cfInfo = crossfader<25?'A dominante':crossfader>75?'B dominante':'mezcla central';
    const queueInfo = queue.length>0 ? queue.map(t=>'+t.title+').join(', ') : 'vacía';
    const system = [
      'Eres DJ_IA, un DJ profesional con 20 años de experiencia en clubes nocturnos latinoamericanos. Experto en lectura de públicos, gestión de energía y transiciones perfectas. Hablas español con naturalidad y carisma de DJ.',
      '',
      'TU TRABAJO:',
